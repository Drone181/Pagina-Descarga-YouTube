...
# ... (resto de las rutas anteriores permanecen igual)
@app.before_request
def before_request():
    logging.info('Before request...')
...
def descarga():
    ...
    if not download_info:
        logging.error("No download information found in session")
        return redirect(url_for('index', error='No hay información de descarga disponible'))
    ...
    # Configuración de headers robusta
    request_headers = {
    ...
    try:
        ...
        with requests.get(video_url, headers=request_headers, stream=True, timeout=10) as response:
            ...
            if 'content-length' in response.headers:
                download_headers['Content-Length'] = response.headers['content-length']
            ...
            def generate():
                try:
                    ...
                    logging.error(f"Error during chunk streaming: {str(e)}")
                    return
